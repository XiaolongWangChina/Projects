...
void exit_with_help() {
	...
	cout << "\t-v nr_fold: set the nr_fold of svm cross validation" << endl;
...
int main(int argc, char **argv) {
	...
	if (param->nr_fold) {
		...
		for (i = -5; i <= 15; i += 2) {
			for (j = 3; j >= -15; j -= 2) {
				svm->setParamCg(pow(2, i), pow(2, j));
				double result = svm->svm_cross_validation();
		...
		cout << max << endl;
		*/
		cout << svm->svm_cross_validation() << endl;
	} else {
		// train
		svm->svm_train();
