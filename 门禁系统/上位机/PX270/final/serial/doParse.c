...
extern int unlockFlag;
extern char alert[];
extern int verifyFailed;

int getDoorStateFlag()
{
	if(doorState != 0 && doorState != 1 && doorState == 2)
		doorState=2;
	return doorState;
}
...
void RepalyPswdAuth(char* userID , char* password)
{
	...
	if(user)//已找到这个用户信息
	{
		if(passwdEqual(user->passwd,password))//密码验证通过
		{
			...
			my_tty_write(replyPswdOk,16);

			
			sprintf(alert,"%s%d%d%d%d%s%d%d%d%d%d%d%s%d","Password Authed OK! ID=",userID[0],userID[1],userID[2],userID[3]," Password=",password[0],password[1],password[2],password[3],password[4],password[5]," FgNum=",fgNum);


	printf("Password Authed OK!\n");
...
verifyFailed = 1;
	...
	sprintf(alert,"%s%d%d%d%d%s%d%d%d%d%d%d%s%d","Password Authed Fail! ID=",userID[0],userID[1],userID[2],userID[3]," Password=",password[0],password[1],password[2],password[3],password[4],password[5]," FgNum=",0xff);

	my_tty_write(replyPswdFail,16);


	printf("Password Authed Fail !\n");
...
void ModifyPasswd(char* userID , char* newPasswd)
{
	...
	if(user)//已找到这个用户信息
	{
		...
		my_tty_write(modifyPswdOK,16);
		
		sprintf(alert,"%s%d%d%d%d%s%d%d%d%d%d%d%s%d","Password Modify OK! ID=",userID[0],userID[1],userID[2],userID[3]," New Password=",newPasswd[0],newPasswd[1],newPasswd[2],newPasswd[3],newPasswd[4],newPasswd[5]," FgNum=",user->fgNum);
	...
	else
	{
		...
		my_tty_write(modifyPswdFail,16);
		sprintf(alert,"%s%d%d%d%d%s%d%d%d%d%d%d%s%d","Password Modify OK! ID=",userID[0],userID[1],userID[2],userID[3]," New Password=",newPasswd[0],newPasswd[1],newPasswd[2],newPasswd[3],newPasswd[4],newPasswd[5]," FgNum=",user->fgNum);
...
int ReplyFGAuthed(char FgNum)
{
	...
	if(!user){
		//没有这个指纹号
		sprintf(alert,"%s%d","Reply Fingure Authed. No such FgNum! FgNum=",FgNum);
	...
	sprintf(alert,"%s%d%d%d%d%s%d","Reply Fingure Authed. ID=",user->uid[0],user->uid[1],user->uid[2],user->uid[3]," FgNum=",FgNum);	
	my_tty_write(replyFgAuthed,16);
...
void ModifyFgNum(char* userID, char fgNum)
{
	...
	{
		user->fgNum = fgNum;
		char modifyFgNumOK[16]={0xAA,0xFF,0x07,0x03,0xCE,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF};
		my_tty_write( modifyFgNumOK ,16);
		sprintf(alert,"%s%d%d%d%d%s%d","Modify FgNum OK. ID=",user->uid[0],user->uid[1],user->uid[2],user->uid[3]," FgNum=",fgNum);		

	}else{

		char modifyFgNumFail[16]={0xAA,0xFF,0x07,0x03,0xCE,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF};
		my_tty_write( modifyFgNumFail ,16);
		sprintf(alert,"%s%d%d%d%d","Modify FgNum Fail No ID=",user->uid[0],user->uid[1],user->uid[2],user->uid[3]);	
		//没找到这个用户号。
	}
