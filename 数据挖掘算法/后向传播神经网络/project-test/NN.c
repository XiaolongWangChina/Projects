...
int main(int argc, char **argv) {
	...
	while (1) {
		...
		/////////////////////////////////////////////////////////////////
		// using the testing.txt to test the BPNN model
		// read the testing data from testing data file into memory
		printf("\n\nUsing the data from testing.txt to test the generated model:\n");
		DataNode **testing_data = load_data_from_file(testfile, test_record_num, attr_per_record);
		int *test_classes = (int *)malloc(sizeof(int) * test_record_num);
		DataNode *test_classname = (DataNode *)malloc(sizeof(DataNode) * test_record_num);
		double **normal_testing_data = preprocessing(testing_data, test_record_num, attr_per_record, test_classes, test_classname);
		...
		// testing the BPNN model
		int miss_matched = 0;
		for (i = 0; i < test_record_num; i++) {
			// set the input units
			for (j = 0; j < attr_per_record - 2; j++) {
				bpnn->input_units[j] = normal_testing_data[i][j+2];
			}
			// one test
			bpnn_test(bpnn);
			...
			confusion_matrix[test_classes[i]][c_no]++;

			// set miss matched
			if (test_classes[i] == c_no) {
			} else {
				miss_matched++;
			}
		}

		double miss_classify_rate = (double)miss_matched/(double)test_record_num;
		printf("error rate: %lf\n", miss_classify_rate);
		printf("classification accuracy: %lf\n", 1 - miss_classify_rate);
		...
		printf("\t");
		for (i = 0; i < class_num; i++) {
			printf("%s\t", test_classname[i].data_value);
		}
		printf("\n");
		for (i = 0; i < class_num; i++) {
			printf("%s\t", test_classname[i].data_value);
		...
		for (i = 0; i < test_record_num; i++) {
			free(testing_data[i]);
			free(normal_testing_data[i]);
		}
		free(testing_data);
		free(normal_testing_data);
		free(test_classes);
		free(test_classname);
		/////////////////////////////////////////////////////////////////
